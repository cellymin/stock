			...
			<form id="tab" method="post" action="" autocomplete="off">
				<table width="100%">
					<tr>
						<td width="400">
							<label>登录名</label>
							<input type="text" name="user_name" value="<{$user.user_name}>" class="input-xlarge" autofocus="true" required="true" >
						</td>
						<td>
							<label>公司</label>
							<{html_options name=companyId id="companyId" class="input-xlarge" options=$company_options selected=$user.companyId required="true"}>
						</td>
					</tr>
					<tr>
						<td>
						...
						<td>
							<label>部门</label>
							<{html_options name=departmentId id="departmentId" class="input-xlarge" options=$department_options selected=$user.departmentId required="true"}>
						</td>
					</tr>
					<tr>
						<td>
						...
						<td>
							<label>员工</label>
							<{html_options name=employeeId id="employeeId" class="input-xlarge" options=$employee_options selected=$user.employeeId required="true"}>
						</td>
					</tr>
					<tr>
						<td>
					...
					<tr>
						...
						<td>
							...
							<select name="selectAll" class="input-xlarge">
								<option value="0" <{if $user.selectAll==0}>selected<{/if}>>查看所属公司</option>
								<option value="1" <{if $user.selectAll==1}>selected<{/if}>>全部公司</option>
							</select>
						</td>
					</tr>
					<tr>

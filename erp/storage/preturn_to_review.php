...
if (Common::isPost()) {
    if ($nonceStr == $_SESSION[UserSession::SESSION_NAME]['form_nonceStr']) {
        if (!empty($orderId)
            || in_array($type,
                array('PLAN', 'ARRIVAL', 'RETURN', 'PURCHASE_IN', 'ALLOT_IN', 'ALLOT_OUT', 'USE_OUT', 'INVENTORY','PURCHASE_RETURN'))
        ) {
...
$rs = $client->request('Reviewer_Options.Go', array('type' => 'PURCHASE_IN'));
if ($client->getRet() == PhalApiClient::RET_OK) {
...
Template::assign('url', 'preturn_to_review.php');

...
class Model_OrderGoods extends PhalApi_Model_NotORM
{
    ...
    public function getTableName($id)
    {
        $tableName = array(
            ...
            'PURCHASE_RETURN'=> 'orders_ipth_goods'
        );
    ...
    public function getList($orderId, $start, $page_size)
    {
        ...
        if (in_array($type, array('PLAN', 'RETURN', 'ARRIVAL', 'PURCHASE_IN', 'ALLOT_IN','PURCHASE_RETURN'))) {
            $columns .= 'u.unitName,';
            $left_join .= 'left join vich_goods_units u on u.unitId=g.goodsUnitId ';
        }

        if (in_array($type,
            array('PURCHASE_IN', 'ALLOT_IN', 'ALLOT_OUT', 'USE_OUT', 'INVENTORY', 'OTHER_IN', 'OTHER_OUT','SALE_OUT','SALE_RETURN'))) {
    ...
    public function getIpGoods($orderId){
        $param[':orderId'] = $orderId;
        $sql = "SELECT og.* FROM vich_orders_ip ip
                INNER JOIN vich_orders_ipth th ON ip.orderNo = th.contactNo
                LEFT JOIN vich_orders_ip_goods og ON ip.orderId = og.orderId
                WHERE og.flag=1 AND ip.flag=3 AND th.orderId = :orderId ";
        return $this->getORM()
            ->queryAll($sql, $param);
    }
    //可能有不同批次的同一个商品
    public function getPurchaseBatchNo($orderId,$goodsId,$batchNo){
        $param[':orderId'] = $orderId;
        $param[':goodsId'] = $goodsId;
        $param[':batchNo'] = $batchNo;
        $sql = "SELECT og.goodsCnt FROM vich_orders_ip ip
                LEFT JOIN vich_orders_ipth th ON ip.orderNo = th.contactNo
                LEFT JOIN vich_orders_ip_goods og ON ip.orderId = og.orderId
                WHERE og.flag=1 AND ip.flag=3 AND th.orderId = :orderId AND og.goodsId =:goodsId AND og.orderSubNo = :batchNo";
        return $this->getORM()
            ->queryAll($sql, $param);
    }
